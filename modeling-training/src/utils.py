...
def make_hmm_data(data_dir, cols, sort=True, first_n=1000):
    for file in glob.glob(data_dir + "*")[0:1]:
        df = pd.read_csv(file)
        print(df.columns)
    if sort:
        try:
...
def get_stats_for_cnn(file_pths, has_loss=False):
    ...
    for file_pth in file_pths:
        ...
        if has_loss:
            ...
            # buf["train_accuracy"].append(data["train_accuracy"])
            # buf["eval_accuracy"].append(data["eval_accuracy"])
            buf["train_minority_loss"].append(data["train_minority_loss"])
            buf["train_majority_loss"].append(data["train_majority_loss"])
            buf["eval_minority_loss"].append(data["eval_minority_loss"])
            buf["eval_majority_loss"].append(data["eval_majority_loss"])
    return buf
